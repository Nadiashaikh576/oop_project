...
do{ console.log(chalk.yellowBright.italic.bold("\n\tWelcome Student!\t\n"));
    const answer = await inquirer.prompt({
        name: "select",
        type: "list",
        message: chalk.redBright("With whom would you like to interact?"),
        choices: [chalk.magentaBright("Staff"), chalk.magentaBright("Student"), chalk.magentaBright("Exit")]
    })
    ...
    } else if(answer.select == chalk.magentaBright("Student")){
        const ans = await inquirer.prompt({
            name: "student",
            type: "input",
            message: chalk.cyanBright("Please enter the Student's name, you want to engage with:")
        })
